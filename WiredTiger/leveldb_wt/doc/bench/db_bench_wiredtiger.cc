...
static int FLAGS_cache_size = (64 * 1024 * 1024);
...
int main(int argc, char** argv) {
  ...
  int cpu_start = 0, cpu_end = 0, cpu_idx = 0;
  int cpu_start_2 = 0, cpu_end_2 = 0;
  cpu_set_t cpuset;
  pthread_t thread;

  thread = pthread_self();

  sprintf(ioctl_file_name, "/mnt/pmem0/ioctl_file_%d\n", getpid());
  ...
  if (ioctl_ret >= 0) {
      ...
      for (cpu_idx = cpu_start_2; cpu_idx < cpu_end_2; cpu_idx++) {
          CPU_SET(cpu_idx, &cpuset);
      }
