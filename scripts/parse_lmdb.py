...
# Usage: python3 parse_lmdb.py <num_fs> <fs1> <fs2> .. <num_runs> <start_run_id> <result_dir> <output_file>
...
def parse_file(filename, perf_list):
    ...
    for line in lines:
        if 'fillseqbatch' in line:
            words = line.split()
            perf = perf + float(words[6])
...
def main():
    ...
    for workload in workloads:
        ...
        for run in runs:
            ...
            for filesystem in fs:
                result_file = result_dir + '/' + filesystem + '/lmdb/' + 'fillseqbatch_Run' + str(run) + '.out'
