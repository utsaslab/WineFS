...
def main():
    if len(sys.argv) < 6:
        print('Usage: python3 parse_wiredtiger.py <num_fs> <fs1> <fs2> .. <num_runs> <start_run_id> <result_dir> <output_csv_file)')
        return
    ...
    workloads = ['fillrandom,readrandom_']
    ...
    for workload in workloads:
        ...
        for run in runs:
            ...
            for filesystem in fs:
                result_file = result_dir + '/' + filesystem + '/wiredtiger/' + workload + 'Run' + str(run) + '.out'
