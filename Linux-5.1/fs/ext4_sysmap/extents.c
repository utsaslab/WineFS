...
out2:
	...
	EXT4_END_TIMING(allocate_blocks_t, allocate_blocks_time);
	return err ? err : allocated;
}
...
retry:
	while (ret >= 0 && len) {
        ...
        if (!(current->flags & PF_USE_HUGEPAGES) && (map.m_pblk % 512 == 0)) {
            if (EXT4_SB(inode->i_sb)->s_mb_free_pending) {
                jbd2_journal_force_commit_nested(EXT4_SB(inode->i_sb)->s_journal);
            }
        }
...
long ext4_fallocate(struct file *file, int mode, loff_t offset, loff_t len)
{
	...
	EXT4_START_TIMING(fallocate_t, fallocate_time);
	...
	if (IS_ENCRYPTED(inode) &&
		...
		EXT4_END_TIMING(fallocate_t, fallocate_time);
		return -EOPNOTSUPP;
	}
