...
out:
	...
	case EXT4_PRINT_TIMING:
		ext4_print_timing_stats(sb);
		dax_print_timing_stats(sb);
		return 0;
	case EXT4_CLEAR_STATS:
		ext4_clear_stats(sb);
		dax_clear_stats(sb);
		return 0;
	case EXT4_CLEAR_NUMA_STATS:
	  atomic64_set(&(sb->remote_data), 0);
	  atomic64_set(&(sb->local_data), 0);
		return 0;
	case EXT4_PRINT_NUMA_STATS:
	        printk(KERN_INFO " ----- NUMA STATS ----- \n");
		printk(KERN_INFO " remote data transfers = %lld\n", sb->remote_data.counter);
		printk(KERN_INFO " local data transfers = %lld\n", sb->local_data.counter);
		printk(KERN_INFO " ---------------------- \n");
		return 0;
	default:
		return -ENOTTY;
