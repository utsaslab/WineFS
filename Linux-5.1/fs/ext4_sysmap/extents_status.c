...
{
	ext4_es_cachep = kmem_cache_create("ext4_sysmap_extent_status",
			  ...
			  unsigned int status)
{
	...
	int err = 0;
	struct ext4_sb_info *sbi = EXT4_SB(inode->i_sb);
	INIT_TIMING(add_blocks_es_time);
	...
	EXT4_START_TIMING(add_blocks_es_t, add_blocks_es_time);	
...
error:
	...
	EXT4_END_TIMING(add_blocks_es_t, add_blocks_es_time);	
	return err;
}
...
{
	ext4_pending_cachep = kmem_cache_create("ext4_sysmap_pending_reservation",
