...
void main() {
    ...
    ret = ioctl(fd, 0xBCD00010, &hugepages_flag_value);
    if (ret < 0) {
        printf("%s: ioctl failed. Error = %s\n", __func__, strerror(errno));
        exit(-1);
    } else {
        printf("%s: ioctl passed. Will print timing\n", __func__, ret);
    }

    ret = ioctl(fd, 0xBCD00011, &hugepages_flag_value);
    if (ret < 0) {
        printf("%s: ioctl failed. Error = %s\n", __func__, strerror(errno));
        exit(-1);
    } else {
        printf("%s: ioctl passed. Will clear stats\n", __func__, ret);
    }
